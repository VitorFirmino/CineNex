...
import { Loader2, Mail, Lock, Eye, EyeOff, AlertCircle, User, Check, X } from 'lucide-react';
    ...
    <div className="w-full max-w-md mx-auto p-6 space-y-6 bg-zinc-950/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl">
      ...
      <form noValidate onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="space-y-2">
          ...
          {errors.name && (
            <span id="name-error" className="text-xs text-red-400 ml-1">{errors.name.message}</span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {errors.email && (
            <span id="email-error" className="text-xs text-red-400 ml-1">{errors.email.message}</span>
          )}
        </div>
        ...
        <Button
          type="submit"
          disabled={isPending}
          className="w-full bg-gradient-to-r from-emerald-400 via-emerald-600 to-teal-700 hover:from-emerald-300 hover:via-emerald-500 hover:to-teal-600 shadow-[0_0_25px_var(--glow-20)] hover:shadow-[0_0_35px_var(--glow-40)] text-white font-black py-6 rounded-xl transition-all active:scale-[0.98] uppercase tracking-widest text-xs"
        >
          {isPending ? (
      ...
      <div className="text-center text-sm text-zinc-500">
        Já tem uma conta? <Link href="/login" className="text-emerald-400 hover:underline">Entre agora</Link>
      </div>
    </div>
  );
}
